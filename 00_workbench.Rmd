...
# news_true <- news_true %>% filter(nchar(text) >= 60)
# news_fake <- news_fake %>% filter(nchar(text) >= 60)
```
...
one_sentence_fake <- one_sentence_fake[nchar(one_sentence_fake$first_sentence) > 100, ]
...
sentence_fake <- sentence_fake[nchar(sentence_fake$first_sentence) > 100, ]
